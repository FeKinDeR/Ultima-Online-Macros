...
#
#
old_utils.RESPOND_AFK_GUMP = False
DISCORD_WEBHOOK_HTOWN = "https://discord.com/api/webhooks/1296879135452299346/BnAu3MaQE88cNb8JqBhmIb4T7aFW0FU7xvWk0iK7wd6ZNbTGRcnZjkydQ3KIcZhljjXh"
old_utils.DISCORD_WEBHOOK_AFK_URL = DISCORD_WEBHOOK_HTOWN
#
old_utils.DISCORD_WEBHOOK = DISCORD_WEBHOOK_HTOWN
old_utils.WAV_AFK = "\\Scripts\\Sounds\\AFKCHECK.wav"
#
...
timer_healpot = datetime.now()- timedelta(seconds=8)
#timer_buffhealing = datetime.now()- timedelta(seconds=8)
timer_apple = datetime.now()- timedelta(seconds=120000) 
alert_mortal = True  
#
#
# =====================================================================================================
...
def check_in_jail():
    ...
    if (currentX >= JAIL_X_TOP_LEFT) and (currentX <= JAIL_X_BOTTOM_RIGHT):
        if (currentY >= JAIL_Y_TOP_LEFT) and (currentY <= JAIL_Y_BOTTOM_RIGHT):
            PlayWav(ALARM)
            send_discord_message(old_utils.DISCORD_WEBHOOK,'**HALLOWEEN** | **[BOT:** ' + CharName() + '**] ->** movido para Jail ! ' + GetName(FindItem()))
            exit()
            return True
    else:
        return False
...
def find_boss(range=18):
    ...
    names_mobs = ['Gravedigger','Jack The Ripper','Jack Frost','The Gravedigger','Headless Horseman','A Costumed Orc Champion','Freddy Krueger','Helda','Araneam','Britomartis','A Steaming Heap','Peinsluth','A Pumpkin Lord','Mr. Hyde','Wraith High Priest','The Guardian of The Asylum','Clawser Jr','Clawser','A Noxious Gas Cloud','Gomez', 'Terror', 'Suspicion', 'Panic', 'Paranoia', 'Despair', 'Revulsion', 'Medusa','The Potential Horror','The Image of Your','The Memory of','Santa Claws','Father Flog','The White Witch','Christmas Angel','The Grinch','Maugrim','Krampus']
    types_mobs = [0x190, 0x191, 0x2df, 0x29b, 0x2d5, 0xf, 0x53, 0x10b, 0x33e, 0x13e, 0x1e, 0x49, 0x111, 0x0109, 0x0315, 0x00A8, 0x0309, 0x02E6, 0x02C0, 0x2d8,0x97,0x35,0x310,0x25e,0xb0,0x25d,0x309,0x115]
...
def survive():
    ...
    while True:
        ...
        if IsPoisoned(Self()):
            UseType(0xf07,0) 
            Wait(600)
        if not IsYellowHits(Self()) and GetHP(Self()) < GetMaxHP(Self())*0.7:
            if datetime.now() >= timer_healpot +timedelta(seconds=8):
                UseType(0xf0c,0)
                timer_healpot = datetime.now()
                ClientPrintEx(Self(),62,0,"Heal pot!!") 
        ...
        # ---------------------------------------------------------------------
        if GetStam(Self()) < GetMaxStam(Self())*0.7:
            if FindType(0xf0b,Backpack()):
                if(FindQuantity() > 3):
                    UseType(0xf0b,0)
                    Wait(600)
                    ClientPrintEx(Self(),62,0,"Refresh pot!!")
...
def attack_boss():
    ...
    if bossID != None and IsDead(bossID) == False and GetDistance(bossID) < 2:
        keepDistance(bossID,5)
    # ---------------------------------------------------------------------
    # --------------------------- AGI / STR -------------------------------
    # ---------------------------------------------------------------------
    if GetDistance(bossID) < 20:
        if GetStr(Self()) < status_str: 
            if UseType(0xf09,0):
                Wait(600)
        if GetDex(Self()) < status_dex:  
            if UseType(0xf08,0):
                Wait(600)
        #TODO CHECK CURSE
...
if __name__ == '__main__':
    ...
    survive_thread = threading.Thread(target=survive, daemon=True)
